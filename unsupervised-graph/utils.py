...
def evaluate_node_classification_linearmodel(embeddings, data, name, search=True):
    ...
    for i in range(20):
        ...
        if search:
            ...
            classifier = GridSearchCV(LinearSVC(), params, cv=pds, scoring='accuracy', verbose=0)
        else:
            classifier = LinearSVC(C=10)
...
def evaluate_node_classification_LR_model(embeddings, data, name, search=True):
    ...
    for i in range(20):
        ...
        if search:
            params = {'C':[0.0001, 0.001, 0.01, 0.1, 1, 10, 100, 1000]}
            classifier = GridSearchCV(LogisticRegression(multi_class='multinomial', solver='lbfgs'), params, cv=pds, scoring='accuracy', verbose=0)
        else:
...
def evaluate_node_classification_RF_model(embeddings, data, name, search=True):
    ...
    for i in range(20):
        ...
        if search:
            params = {'n_estimators': [100, 200, 500, 1000]}
            classifier = GridSearchCV(RandomForestClassifier(), params, cv=pds, scoring='accuracy', verbose=0)
        else:
            classifier = RandomForestClassifier()
