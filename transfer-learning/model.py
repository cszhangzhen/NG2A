...
class GNN(torch.nn.Module):
    ...
    def __init__(self, num_layer, emb_dim, JK = "last", drop_ratio = 0, gnn_type = "gin"):
        ...
        self.proj_head = nn.Sequential(nn.Linear(emb_dim, emb_dim), nn.ReLU(inplace=True), nn.Linear(emb_dim, emb_dim))
    ...
    def forward(self, *argv):
        ...
        for layer in range(self.num_layer):
            ...
            hp = self.gan(h.detach(), edge_index, batch, edge_attr)
            h_list.append(h)
            hp_list.append(hp)
        ...
        if self.JK == "concat":
            ...
            node_representation_pos = hp_list[-1]
        elif self.JK == "max":
        ...
        return node_representation, node_representation_pos
    
    def forward_pred(self, *argv):
        if len(argv) == 4:
            x, edge_index, edge_attr, batch = argv[0], argv[1], argv[2], argv[3]
        elif len(argv) == 1:
            data = argv[0]
    ...
    def forward_cl(self, data):
        x, x_pos = self.forward(data)
        x = self.proj_head(global_add_pool(x, data.batch))
        x_pos = self.proj_head(global_add_pool(x_pos, data.batch))
...
class GNN_graphpred(torch.nn.Module):
    ...
    def forward(self, *argv):
        ...
        node_representation = self.gnn.forward_pred(x, edge_index, edge_attr, batch)
