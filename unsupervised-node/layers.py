...
class Discriminator(torch.nn.Module):
    def __init__(self, args):
        ...
        self.fc1 = torch.nn.Linear(self.nhid, self.nhid // 2)
        self.fc2 = torch.nn.Linear(self.nhid // 2, 1)
...
class GAN(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch):
        ...
        pos_loss = - torch.log(self.generator.discriminate(x_gen, x) + 1e-15).mean()
        neg_loss = - torch.log(1 - self.generator.discriminate(x_gen, x_neg) + 1e-15).mean()
